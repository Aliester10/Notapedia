        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-xs uppercase text-slate-500 border-b border-slate-200">
            <tr>
              ...
              <th className="px-4 py-3 text-right font-medium">Sisa Qty</th>
          ...
          <tbody>
            {hasil.map((po) => {
              const totalSisa = po.items.reduce(
                (s, it) => s + (it.qty_pesan - it.qty_terkirim),
                0
              );
              return (
                <tr key={po.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                  <td className="px-4 py-3 font-medium text-slate-800">{po.no_po}</td>
                  <td className="px-4 py-3 text-slate-600">{po.client_nama}</td>
                  <td className="px-4 py-3 text-slate-600">{formatDate(po.tanggal_po)}</td>
                  <td className="px-4 py-3 text-right text-slate-600">{po.items.length}</td>
