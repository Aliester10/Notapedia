...
export default function PODetail() {
  ...
  return (
    <div>
      ...
      <PageHeader
        title={po.no_po}
        subtitle={`${po.client_nama} · ${formatDate(po.tanggal_po)}`}
        actions={
          ...
          <table className="w-full text-sm">
            <thead className="text-xs text-slate-500 border-b border-slate-200">
              <tr>
                ...
                <th className="text-right py-2 font-medium w-28">Terkirim</th>
                <th className="text-right py-2 font-medium w-28">Sisa</th>
              </tr>
            </thead>
            <tbody>
              {po.items.map((it) => {
                ...
                return (
                  <tr key={it.id} className="border-b border-slate-100 last:border-0">
                    ...
                    <td className="py-3 text-center text-slate-600">{it.satuan}</td>
                    <td className="py-3 text-right text-slate-600">{formatNumber(it.qty_pesan)}</td>
                    <td className="py-3 text-right text-slate-600">{formatNumber(it.qty_terkirim)}</td>
        ...
        <div className="card p-5">
          ...
          <dl className="text-sm space-y-2.5">
            ...
            <div className="flex justify-between">
              ...
              <dd className="font-medium text-slate-800">{formatDate(po.tanggal_po)}</dd>
            </div>
            <div className="flex justify-between">
