...
function seed(d) {
  const tx = d.transaction(() => {
    ...
    const insInvItem = d.prepare('INSERT INTO invoice_item (invoice_id, nama_barang, satuan, qty, harga_satuan, subtotal) VALUES (?,?,?,?,?,?)');
    ...
    insInvItem.run(v1, 'Besi Beton 12mm', 'batang', 80, 85000, 6800000);
    insInvItem.run(v1, 'Semen Portland 50kg', 'sak', 100, 75000, 7500000);

    const v2 = insInv.run('INV-0002', p1, s2, '2026-08-09', 'Ditagih', '2026-08-15', null, 4450000, '2026-08-09 09:00:00').lastInsertRowid;
    insInvItem.run(v2, 'Besi Beton 12mm', 'batang', 40, 85000, 3400000);
    insInvItem.run(v2, 'Pasir Urug', 'kubik', 3, 350000, 1050000);

    const v3 = insInv.run('INV-0003', p2, s3, '2026-08-10', 'Dibayar', '2026-08-18', '2026-08-25', 8200000, '2026-08-10 09:00:00').lastInsertRowid;
    insInvItem.run(v3, 'Cat Tembok 25kg', 'kaleng', 30, 250000, 7500000);
    insInvItem.run(v3, 'Kuas Roll 9 inch', 'pcs', 20, 35000, 700000);

    const v4 = insInv.run('INV-0004', p3, s4, '2026-08-14', 'Terkirim', null, null, 3800000, '2026-08-14 09:00:00').lastInsertRowid;
    insInvItem.run(v4, 'Triplek 9mm', 'lembar', 40, 95000, 3800000);
